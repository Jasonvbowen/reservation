...
public class Search {
    ...
    public static ArrayList<Comparable> binarySearch(Comparable[] arr, Comparable searchValue) {
     ...
     */
    private static void binarySearch(ArrayList<Comparable> arr, Comparable searchValue, int low, int high) {
        if (high <= low) {
            return;
        }
        int mid = (high + low) / 2;
    ...
    public static ArrayList<Comparable> binarySearchFirstLetter(Comparable[] arr, char searchedLetter) {
        ArrayList<Comparable> comp = new ArrayList<>();
        comp.addAll(Arrays.asList(arr));
        searchList = new ArrayList<>();
        binarySearchFirstLetter(comp, Character.toLowerCase(searchedLetter), 0, comp.size());
        return searchList;
    }
     ...
     * @return integer value of the array that the customer name is in
     */
    private static void binarySearchFirstLetter(ArrayList<Comparable> arr, char searchedLetter, int low, int high) {
        if (high <= low) {
            return;
        }
        int mid = (high + low) / 2;
